...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        ...
        for slip in self:
            contract = slip.contract_id or False
            gross = contract.wage if contract else slip.line_ids.filtered(lambda l: l.code == 'GROSS') and slip.line_ids.filtered(lambda l: l.code == 'GROSS')[0].total or 0.0
            self.env['employee.salary.report'].create({
                ...
                'gross_salary': gross,
                'other_deduction':slip.unpaid_amount,
                'total_days_in_month':slip.total_days_in_month ,
                'total_working_days_in_month':slip.total_working_days_in_month
            })
        return res
